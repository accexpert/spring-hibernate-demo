 ...
 */
@Component
public class Demo2DbHandler2 {
    private static final Logger LOGGER = Logger.getLogger(Demo2DbHandler2.class);
    @PersistenceContext(unitName = ApplicationConfig.PERSISTENCE_UNIT_NAME)
    private EntityManager em;

    public Demo2DbHandler2() {
        LOGGER.info(this.getClass().getSimpleName()+" created");
    }

    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void saveRecordSecondAttachedTransaction() {
        LOGGER.info("Enter second transaction with propagation");
        UsersEntity e = new UsersEntity();
        em.persist(e);
        LOGGER.info("Exit second transaction with propagation");
    }
